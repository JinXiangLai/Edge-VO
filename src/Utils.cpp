...
vector<Eigen::Vector2d> FindMatchesAlongEpipolar(const Landmark &lk1, const KeyFrame &kf2, const Pose &T21, const Camera &cam) {
...
void GetProjectRange(const Landmark &lp, const Pose& T21, const Camera &cam, Eigen::Vector2i &xRange, 
    ...
    if(xLen < config->minEpipolarLength) {
        const int diff = config->minEpipolarLength - xLen;
    ...
    if(yLen < config->minEpipolarLength) {
        const int diff = config->minEpipolarLength - yLen;

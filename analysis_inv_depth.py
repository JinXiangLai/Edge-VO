...
def read_and_plot_depth_data(filename, save_folder: str):
    ...
    for point_id, group_data in grouped:
        ...
        fig.suptitle(f'Inverse Depth Analysis - PointID: {point_id}',
                     fontsize=14,
                     fontweight='bold')
        ...
        if valid_true_depth.any():
            ...
            label_y = first_true_value + (y_range[1] -
                                          y_range[0]) * 0.02  # 稍微向上偏移

            ax1.text(first_true_time,
                     label_y,
                     f'True Depth: {first_true_value:.3f}',
                     ha='right',
                     va='bottom',
                     fontsize=10,
                     fontweight='bold',
        ...
        # 保存图片
        output_filename = os.path.join(save_folder,
        ...
        if valid_true_depth.any():
            true_depth_val = true_depths[valid_true_depth].iloc[
                0] if valid_true_depth.any() else 0
    ...
    if len(valid_data) > 0:
        ...
        print(
            f"Mean absolute error: {valid_data['depthDiff'].abs().mean():.3f}")
...
def create_summary_plot(df, save_folder):
    ...
    fig.suptitle('Inverse Depth Filter Summary Analysis',
                 fontsize=16,
                 fontweight='bold')
    ...
    valid_data = df[df['trueDepth'] > 1e-6]
    if len(valid_data) > 0:
        ax1.hist(valid_data['depthDiff'].abs(),
                 bins=20,
                 alpha=0.7,
                 color='skyblue',
                 edgecolor='black')

...
int main(int argc, char** argv) {
    ...
    for (int i = firstImgIdx; i < vTimeStamps.size(); ++i) {
        ...
        cout << "accDist: " << accDist << " | ";
        accDist += Tc1c2.t_wb_.norm();
        cout << accDist << endl;
        ...
        if (!isInitialized) {
            ...
            if (kfConvergeEdgeRatio > 0.6 ||
                (accDist > 1.0 && (curF.id_ - initFrame->id_ > 30) &&
                 kfConvergeEdgeRatio > 0.3)) {
        ...
        // TODO: 如果pose存在误差，这样更新就会产生错误的landmark估计值了
        const double kfConvergeEdgeRatio = win.back()->UpdateDepth(curF);
#else
        // step1
        const double kfConvergeEdgeRatio = win.back()->UpdateDepth(curF);
        ...
        cout << "curF depth map initialized depth ratio: " << initDepthRatio
             << endl;
